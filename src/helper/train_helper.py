...
class CustomSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
  ...
  def __init__(self, train_lr_param_k, d_model, warmup_steps, dtype=None):
  ...
  def __call__(self, step):
    ...
    return self.train_lr_param_k * tf.math.rsqrt(self.d_model) * \
...
def get_bucket_info(batch_total_size, num_gpus, min_bkt, max_bkt, step):
  ...
  bucket_boundaries = []
  bucket_batch_sizes = []
  for boundary in range(min_bkt, max_bkt + step, step):
