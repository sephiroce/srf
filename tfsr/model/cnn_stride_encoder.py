...
class CNNStrideEncoder(tf.keras.Model): #pylint: disable=too-many-ancestors
  """
  An implementation of CNN based CTC networks which uses the same CNN-FE with
  the SRF models
  """
  def get_config(self):
    pass
  ...
  def __init__(self, config, logger, class_n):
    ...
    self.cnn_fe = CapsulationLayer(cnn_n, nfilt, kernel_size, stride, init,
                                   name="conv_feat")
    self.enc_layers = []
    for i in range(4):
    ...
    self.layernorms_proj = \
      [tf.keras.layers.LayerNormalization(epsilon=1e-6, name="proj_ln_%d"%(i+1))
       for i in range(self.proj_layers - 1)]
    self.input_dropout = tf.keras.layers.Dropout(rate=config.train_inp_dropout,
                                                 name="inp_dropout")
  ...
  def call(self, inputs, **kwargs):
    ...
    emb, _, _ = self.cnn_fe(inputs, input_lengths=inp_len)
    ...
    for idx in range(self.enc_num):
      ...
      emb = self.mask([emb, inp_len, self.cnn_n ** self.stride])
    ...
    for idx in range(self.proj_layers - 1):
      ...
      emb = self.mask2([emb, inp_len, self.cnn_n ** self.stride])
